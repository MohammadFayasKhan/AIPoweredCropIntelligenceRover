...
void maintainWebSocketConnection() {
  ...
  #if USE_SECURE_WSS
    wsClient.setInsecure(); // Allow TLS connections without strict root CA bundling
  #endif

  String wsUrl = String(USE_SECURE_WSS ? "wss://" : "ws://") +
                 g_effectiveBackendHost + ":" + String(BACKEND_PORT) +
                 BACKEND_WS_PATH + "?device_id=" + DEVICE_ID +
                 "&token=" + DEVICE_TOKEN;

  Serial.printf("[WS] Connecting outbound to gateway: %s:%d%s\n", g_effectiveBackendHost.c_str(), BACKEND_PORT, BACKEND_WS_PATH);
  ...
  if (connected) {
    ...
    g_wsFailCount++;
    Serial.printf("[WS] Gateway connection to %s:%d failed (fails=%d). Will retry.\n", 
                  g_effectiveBackendHost.c_str(), BACKEND_PORT, g_wsFailCount);
  }
}
